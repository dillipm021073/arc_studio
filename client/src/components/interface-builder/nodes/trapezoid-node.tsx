...
export interface TrapezoidNodeData {
  label: string;
  type: string;
  width: number;
  height: number;
  ...
  fontSize?: number;
  textColor?: string;
  topWidth?: number; // Width of the top edge as percentage of bottom width
  onDelete?: () => void;
  onDuplicate?: () => void;
...
function TrapezoidNode({ data, selected }: NodeProps<TrapezoidNodeData>) {
  ...
  const topWidth = data.topWidth || 0.6; // Top is 60% of bottom width by default
  ...
  const getTrapezoidPoints = () => {
    const topOffset = ((1 - topWidth) * width) / 2;
    
    // Points: top-left, top-right, bottom-right, bottom-left
    const points = [
      `${strokeWidth + topOffset},${strokeWidth}`, // top-left
      `${width - strokeWidth - topOffset},${strokeWidth}`, // top-right
      `${width - strokeWidth},${height - strokeWidth}`, // bottom-right
      `${strokeWidth},${height - strokeWidth}` // bottom-left
    ];
...
export default memo(TrapezoidNode);