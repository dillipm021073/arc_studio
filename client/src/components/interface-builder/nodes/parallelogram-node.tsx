...
export interface ParallelogramNodeData {
  label: string;
  type: string;
  width: number;
  height: number;
  ...
  fontSize?: number;
  textColor?: string;
  skewAngle?: number; // Angle of skew in degrees
  onDelete?: () => void;
  onDuplicate?: () => void;
...
function ParallelogramNode({ data, selected }: NodeProps<ParallelogramNodeData>) {
  ...
  const skewAngle = data.skewAngle || 20; // Default 20 degree skew
  ...
  const getParallelogramPoints = () => {
    ...
    const points = [
      `${strokeWidth + skewOffset},${strokeWidth}`, // top-left
      `${width - strokeWidth},${strokeWidth}`, // top-right
      `${width - strokeWidth - skewOffset},${height - strokeWidth}`, // bottom-right
